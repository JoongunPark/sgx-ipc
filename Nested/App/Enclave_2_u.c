...
static sgx_status_t SGX_CDECL Enclave_2_ocall_print_string(void* pms)
{
...
static sgx_status_t SGX_CDECL Enclave_2_ocall_sleep(void* pms)
{
...
static const struct {
	size_t nr_ocall;
	void * table[2];
} ocall_table_Enclave_2 = {
	2,
	{
		(void*)Enclave_2_ocall_print_string,
		(void*)Enclave_2_ocall_sleep,
	}
};
sgx_status_t inner_2_init(sgx_enclave_id_t eid)
{
	sgx_status_t status;
	status = sgx_ecall(eid, 0, &ocall_table_Enclave_2, NULL);
	return status;
}
...
sgx_status_t ecall_test_consumer(sgx_enclave_id_t eid, void* buf)
{
	...
	ms.ms_buf = buf;
	status = sgx_ecall(eid, 1, &ocall_table_Enclave_2, &ms);
	return status;
}
