...
static sgx_status_t SGX_CDECL sgx_inner_2_init(void* pms)
{
...
static sgx_status_t SGX_CDECL sgx_ecall_test_consumer(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ecall_test_consumer_t));
	ms_ecall_test_consumer_t* ms = SGX_CAST(ms_ecall_test_consumer_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	void* _tmp_buf = ms->ms_buf;


	ecall_test_consumer(_tmp_buf);
...
SGX_EXTERNC const struct {
	...
	2,
	{
		{(void*)(uintptr_t)sgx_inner_2_init, 0},
		{(void*)(uintptr_t)sgx_ecall_test_consumer, 0},
	}
};
